...
const Events = () => {
  ...
  const touchDiffRef = useRef<{ x: number; y: number }>({ x: 0, y: 0 });
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div
          className="relative"
          onTouchStart={hasMultipleEvents ? (e) => {
            const t = e.touches[0];
            touchStartRef.current = { x: t.clientX, y: t.clientY };
            touchDiffRef.current = { x: 0, y: 0 };
          } : undefined}
          onTouchMove={hasMultipleEvents ? (e) => {
            if (!touchStartRef.current) return;
            const t = e.touches[0];
            touchDiffRef.current = {
              x: t.clientX - touchStartRef.current.x,
              y: t.clientY - touchStartRef.current.y,
            };
          } : undefined}
          onTouchEnd={hasMultipleEvents ? () => {
            const { x, y } = touchDiffRef.current;
